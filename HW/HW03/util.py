import heapq, collections, re, sys, time, os, random, itertools
...
class UniformCostSearch(SearchAlgorithm):
    ...
    def solve(self, problem, heuristic=lambda x: 0):
        ...
        frontier.update(startState, heuristic(startState))
        ...
        while True:
            ...
            if problem.isEnd(state):
                ...
                self.visitedStates = tuple(itertools.chain((problem.startState(),), backpointers.keys()))
...
class DynamicProgramming:
    ...
    def __call__(self, state):
        if state in self.future_dict:
            return self.future_dict[state]
        elif self.problem.isEnd(state):
            return 0
        else:
            min_cost = min(itertools.chain(
                [float('inf')],
                (action_cost + self(successor)
                 for action, successor, action_cost in self.problem.succAndCost(state))))
            self.future_dict[state] = min_cost
            return min_cost
